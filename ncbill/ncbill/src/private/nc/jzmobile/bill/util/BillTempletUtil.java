	...
	public static PubBillTempletModel getTemplateID(String billtype)
		...
		try {
			...
			templet = (PubBillTempletModel) dao
					.executeQuery(
	//						"select pk_billtemplet,bill_templetcaption,bill_templetname,pk_billtypecode from pub_billtemplet where isnull(dr,0)=0 and bill_templetcaption='"
	//								+ "MOBILE" + billtype + "' ",
							"select pk_billtemplet,bill_templetcaption,bill_templetname,pk_billtypecode from pub_billtemplet where isnull(dr,0)=0 and bill_templetcaption='"+billtype+"'",
							new ResultSetProcessor() {
